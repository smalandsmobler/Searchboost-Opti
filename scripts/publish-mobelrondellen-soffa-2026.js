#!/usr/bin/env node
/**
 * Publicerar "Bästa soffan 2026"-artikeln till mobelrondellen.se via WordPress REST API.
 * Kör detta skript på EC2 (där SSM-credentials finns tillgängliga).
 *
 * Körs med:  node scripts/publish-mobelrondellen-soffa-2026.js
 *
...
const ARTICLE = {
  title: 'Bästa soffan 2026 — guide för dig som vill ha kvalitet och hållbarhet',
  slug: 'basta-soffa-2026-guide-kvalitet',
  status: 'publish',
  focus_keyword: 'bästa soffan 2026',
  meta_title: 'Bästa soffan 2026 — guide till kvalitet och hållbarhet | Möbelrondellen',
  meta_description: 'Vilken soffa håller länge och ser bra ut 2026? Vi guidar dig genom material, konstruktion och varumärken — så att du väljer rätt soffa för ditt hem och din budget.',
  content: fs.readFileSync(
    path.join(__dirname, '../content-pages/mobelrondellen-basta-soffa-2026.html'),
    'utf8'
  )
    .replace(/<!--[\s\S]*?-->/g, '')
    .replace(/^[\s\S]*<body[^>]*>/i, '')
...
async function findBlogCategory(site) {
  try {
    ...
    // Prioritet: "Blogg" > "Blog" > "Guider" > "Uncategorized" (ID 1)
...
async function main() {
  console.log('=== Möbelrondellen — Bästa soffan 2026 — Publicering ===\n');
