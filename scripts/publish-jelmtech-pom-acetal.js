#!/usr/bin/env node
/**
 * Publicerar POM/acetal-artikeln till jelmtech.se via WordPress REST API.
 * Kör detta skript på EC2 (där SSM-credentials finns tillgängliga).
 *
 * Körs med:  node scripts/publish-jelmtech-pom-acetal.js
 *
...
const ARTICLE = {
  title: 'POM/acetal formsprutning — komplett guide till polyoximetyl​en',
  slug: 'pom-acetal-formsprutning-guide',
  status: 'publish',
  category_id: 48,   // Produktutveckling på jelmtech.se
  focus_keyword: 'POM formsprutning',
  meta_title: 'POM/acetal formsprutning — egenskaper, parametrar och tillämpningar | Jelmtech',
  meta_description: 'Komplett guide till POM (acetal/Delrin): mekaniska egenskaper, formsprutningsparametrar, homopolymer vs kopolymer och när POM väljs framför polyamid eller PP. Från Jelm techs formsprutningsexperter.',
  content: fs.readFileSync(
    path.join(__dirname, '../content-pages/jelmtech-pom-acetal-formsprutning.html'),
    'utf8'
  ).replace(/<!--[\s\S]*?-->/g, '').trim()
};
...
async function main() {
  console.log('=== Jelmtech POM/acetal Publicering ===\n');
