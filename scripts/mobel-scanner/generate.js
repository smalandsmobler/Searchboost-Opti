...
const LEADS_DIR = path.join(ROOT, 'docs/leads/mobel-2026-04-16');
const ANALYSER_DIR = path.join(LEADS_DIR, 'analyser');
const SCAN_RESULTS = path.join(LEADS_DIR, 'scan-results.json');
const RINGLISTA_XLSX = path.join(LEADS_DIR, 'mobel-ringlista-2026-04-16.xlsx');
const SUMMARY_MD = path.join(LEADS_DIR, 'mobel-sammanfattning-2026-04-16.md');
...
async function main() {
  ...
  if (!ONLY) {
    ...
    md += `**Output-mapp:** docs/leads/mobel-2026-04-16/\n\n`;
    ...
    md += `- \`mobel-ringlista-2026-04-16.xlsx\` — sorterad ringlista (lead-score desc)\n`;
