...
## Perispa — Egen WordPress MCP-server (Byggt 2026-04-17)

Ersätter Respira (betalplugin per sajt). Använder standard WP REST API + Application Passwords.

### Vad det gör
- 40 MCP-verktyg (`perispa_*`) för WordPress-hantering direkt i Claude Code
- Multi-site: alla 8 kundsajter via en MCP-server
- Verktyg: sidor, inlägg, media, SEO (Rank Math), menyer, plugins, kategorier, bulk-operationer
- Gratis, obegränsat — ingen proprietär WP-plugin behövs

### Filer
| Fil | Beskrivning |
|-----|-------------|
| `perispa/index.js` | MCP-server (40 verktyg, ~550 rader) |
| `perispa/wp-client.js` | WordPress REST API-klient |
| `perispa/config.js` | Config-laddare |
| `perispa/setup.js` | SSM-import + anslutningstest |
| `~/.perispa/config.json` | Sajt-credentials (genererad från SSM) |
| `.mcp.json` | MCP-servern registrerad för Claude Code |

### Aktiva sajter (8 st, testade OK)
...
### Ny kund? Lägg till i SSM → kör `--from-ssm` → kör om Claude Code
...
## MULTI-SESSION RUTIN (gäller alla sessioner)

### SESSION-START
Claude frågar alltid: **"Vad jobbar vi med idag?"**
Mikael svarar → Claude läser rätt kund-task-fil → låser uppgift → börjar.

### REGLER
**Task-lås:** Skriv `🔒 LÅST [kund | uppgift | tid]` i `kund_{slug}_tasks.md` innan arbete. Frigör vid ✅ eller "pausad".
Andra sessioner hoppar alltid över 🔒-märkta uppgifter.

**Kod-ägande:** Bara deploy-sessionen rör `index.js` / `app.js` / Lambda-filer.
Alla andra sessioner = strikt kund-content (artiklar, WP, mail, task-filer).

**Deploy-lås:** Kontrollera SSM `/seo-mcp/deploy-lock` ALLTID innan SSH/deploy.
Sätt låset → gör jobbet → ta bort det. Lås >2h = stale, ignoreras.

**Git:** `git pull --rebase` innan edit på delade filer. Commit direkt när klart.

**Datorn kör dygnet runt** — sessioner startas autonomt, Mikael behöver inte vara närvarande.

---

## VISION & MOONSHOTS (prioriterad bygglista)

> Dessa mål styr vad vi bygger när ingen specifik uppgift är given.
> Börja alltid med det som genererar pengar eller kunder snabbast.

### BYGGER NU (nästa 1-2 veckor)
1. **searchboost.se total rebuild** — Sveriges snyggaste SEO-byrå-sajt, max konvertering, live case studies från riktiga kunder. Bygg i Replit. Hero + audit-formulär + prispaket + case studies med GSC-grafer.
2. **Daily GSC-mail** — kör 07:00 varje dag, ett mail med alla kunders viktigaste siffror: klick, impressions, delta mot förra veckan, top 3 sökord. AWS SES.
3. **Credential manager UI** — gränssnitt i Opti som läser/skriver SSM. Aldrig leta lösenord igen.
4. **Dashboard UI total rebuild** — modern, kundredo, mobilanpassad. Öppna för kunder när klar.

### BYGGER DENNA MÅNAD
5. **LinkedIn-ghostwriter** — 90 dagars content autonomt per kund, kunden klickar godkänn. 2 000 kr/mån/kund tillägg.
6. **Programmatisk SEO** — auto-generera 100-500 landningssidor per kund baserat på sökdata + produktkatalog.
7. **Video-factory** — varje artikel → YouTube Short + TikTok automatiskt.
8. **Besöksidentifiering** — identifiera vilka FÖRETAG som besöker kundernas sajter. Sälj som tillägg.
9. **Review-maskin** — AI svarar på Google Reviews i kundens röst automatiskt.
10. **GA4 + GTM på alla kunder** — Ferox + Humanpower först, sedan resten.

### MOONSHOTS
11. **White-label Searchboost** — sälj plattformen till andra SEO-byråer.
12. **Automatisk prospektering** — systemet hittar leads, kör audit, skickar cold outreach. Mikael vaknar med bokade möten.
13. **Sitebuilder-as-a-service** — företagsnamn + bransch → komplett SEO-sajt på 30 min.
14. **AI säljagent (röst)** — ringer prospects, pitchar, bokar möte med Mikael.
15. **SEO-domänportfölj** — köp utgångna svenska domäner med hög auktoritet, dirigera mot kunder.
16. **Multilingual** — alla artiklar auto-lokaliseras till norska + danska + finska.
17. **Konkurrensintelligens-abonnemang** — crawla konkurrenters SEO dagligen, sälj rapporten.

### TECH-STACK FÖR BYGGE
- **Frontend:** Replit för snabb prototyp → deploy på EC2 eller Cloudflare Pages
- **Backend:** Befintlig EC2/Express eller nya Cloudflare Workers
- **Data:** BigQuery (redan kört) + $300 GCP-credits
- **AI:** Claude via claude.ai (ingen API-kostnad) — sessioner kör manuellt
- **Automation:** Lambda (befintlig) eller n8n på nya macen
- **Design:** Mörkt premium-tema — `#e91e8c` / `#00d4ff` / `#00e676`
